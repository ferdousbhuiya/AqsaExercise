...
const BANK={
  1:{
    ...
    english:[
      ...
      {p:"Fix the sentence with a capital letter: aqsa likes books.",a:"Aqsa likes books",aliases:["Aqsa likes books."]}
    ],
    science:[
...
function showPage(name){document.querySelectorAll(".page").forEach(p=>p.classList.remove("active"));$(name+"Page").classList.add("active");document.querySelectorAll(".nav-btn").forEach(b=>b.classList.toggle("active",b.dataset.page===name));scrollTo({top:0,behavior:"smooth"});if(name==="home")renderHome();if(name==="progress")renderDashboard();}
...
function startExercise(subject){state.subject=subject;state.questions=shuffle(BANK[state.grade][subject]).slice(0,5);state.index=0;state.correct=0;state.selected="";state.pencil=false;showPage("exercise");renderQuestion()}
...
function checkAnswer(){const q=state.questions[state.index],given=q.choices?state.selected:$("typedAnswer").value;if(!given){showFeedback("Please choose or type an answer.",false);return}const accepted=[q.a,...(q.aliases||[])].map(normalize),ok=accepted.includes(normalize(given));if(ok)state.correct++;showFeedback(ok?"Excellent! That is correct. 🌟":`Good try. The answer is “${q.a}.”`,ok);finishQuestion()}
function showFeedback(text,correct){$("feedback").textContent=text;$("feedback").className=`feedback ${correct?"correct":"incorrect"}`}
function finishQuestion(){$("submitAnswer").classList.add("hidden");$("pencilCorrect").classList.add("hidden");$("pencilTryAgain").classList.add("hidden");$("nextQuestion").classList.remove("hidden");$("questionBar").style.width=`${((state.index+1)/state.questions.length)*100}%`}
