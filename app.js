...
const levelLabel=grade=>YearCurriculum.levelLabel?.(grade)||`Grade ${grade}`;
const levelRevision=grade=>Number(grade)<=2?"mixed-v2":"full-v3";
for(const id of ["gradeSelect","learnGrade","previewGrade","gameGrade"]){const select=document.getElementById(id);if(select&&YearCurriculum.levelOptions)select.innerHTML=YearCurriculum.levelOptions()}
...
const savedLevel=Number(localStorage.getItem("aqsaGrade"));
const state={grade:YearCurriculum.levelIds?.includes(savedLevel)?savedLevel:1,reviewDay:null,subject:null,questions:[],index:0,correct:0,selected:"",pencil:false,range:7};
const $=id=>document.getElementById(id);

function localDate(d=new Date()){return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`}
function today(){return localDate()}
function recordId(){return globalThis.crypto?.randomUUID?.()||`score-${Date.now()}-${Math.random().toString(36).slice(2)}`}
function getRecords(){
 let records;try{records=JSON.parse(localStorage.getItem("aqsaRecords")||"[]")}catch{records=[]}
 if(!Array.isArray(records))records=[];
 let changed=false;records=records.map(r=>{if(r&&typeof r==="object"&&!r.recordId){changed=true;return{...r,recordId:recordId()}}return r}).filter(r=>r&&typeof r==="object");
 if(changed)localStorage.setItem("aqsaRecords",JSON.stringify(records));
 return records;
}
function replaceRecords(records){localStorage.setItem("aqsaRecords",JSON.stringify(records));renderDashboard();renderHome()}
function saveRecord(record){const records=getRecords();records.push({...record,recordId:record.recordId||recordId()});localStorage.setItem("aqsaRecords",JSON.stringify(records));window.CloudSync?.schedule()}
function normalize(v){return String(v??"").trim().toLowerCase().replace(/[.!?]/g,"").replace(/-/g," ").replace(/\s+/g," ")}
function shuffle(items){return [...items].sort(()=>Math.random()-.5)}

function showPage(name){if("speechSynthesis" in window)speechSynthesis.cancel();document.querySelector(".year-path").classList.toggle("hidden",["preview","games","progress"].includes(name));document.querySelectorAll(".page").forEach(p=>p.classList.remove("active"));$(name+"Page").classList.add("active");document.querySelectorAll(".nav-btn").forEach(b=>b.classList.toggle("active",b.dataset.page===name));scrollTo({top:0,behavior:"smooth"});if(name==="home")renderHome();if(name==="learn")renderCurriculum();if(name==="progress")renderDashboard();if(name==="preview")renderPreview();if(name==="games")renderGames();}
document.querySelectorAll("[data-page]").forEach(b=>b.addEventListener("click",()=>showPage(b.dataset.page)));

function renderHome(){
  $("gradeSelect").value=state.grade;
  renderYearControls();
  const records=getRecords(),done=new Set(records.filter(r=>r.revision===levelRevision(state.grade)&&r.grade===state.grade&&r.pathStart===yearStart()&&r.pathDay===selectedDay()&&r.variant===0).map(r=>r.subject));
  $("subjectCards").innerHTML=Object.entries(SUBJECTS).map(([key,s])=>{const pack=currentPack(key);return `<article class="subject-card ${key}"><div class="subject-icon">${s.icon}</div><h3>${s.label}</h3><p>${escapeHtml(pack.unit)}</p><p class="daily-topic">${escapeHtml(pack.lesson.title)}</p><button class="secondary-btn" data-learn="${key}">Learn first</button><div class="subject-meta"><span class="done-badge">${done.has(key)?"✓ Day complete":`${pack.questions.length} mixed questions`}</span><button class="start-btn" data-start="${key}">${done.has(key)?"Review this set":"Start"}</button></div></article>`}).join("");
...
function renderQuestion(){
  ...
  $("questionPassage").textContent=q.passage||"";$("questionPassage").classList.toggle("hidden",!q.passage);$("questionKind").textContent=q.skill+(q.challenge?" · Final challenge":"");$("questionVisual").innerHTML=questionVisual(q);
...
$("speakButton").addEventListener("click",()=>{const q=state.questions[state.index];speak(q.speak||`${q.passage||""} ${q.p}${q.choices?" Options: "+q.choices.join(". "):""}`)});
...
function renderCurriculum(){
 ...
 const sections=(l.sections||[]).map(s=>`<section class="lesson-step"><h4>${escapeHtml(s.title)}</h4><p>${escapeHtml(s.text)}</p></section>`).join("");
 const topics=YearCurriculum.topicsFor?.(state.grade,key)||YearCurriculum.topics[key];
 const library=YearCurriculum.lessonLibrary?.(state.grade,key)||LESSONS[state.grade][key];
 return `<article id="daily-${key}" class="curriculum-card"><h2>${subject.icon} ${subject.label}</h2><p>${escapeHtml(levelLabel(state.grade))} · Day ${pack.day} · ${escapeHtml(pack.unit)}</p>${pack.supported?'<p class="support-badge">Supported VE pathway · shorter steps · repeated practice · age-respectful topics</p>':''}<h3>${escapeHtml(l.title)}</h3><div class="lesson-body"><p>${escapeHtml(l.text)}</p><div class="lesson-visual">${escapeHtml(l.visual).replace(/\n/g,"<br>")}</div><p><strong>Explore today's example:</strong> ${escapeHtml(l.example)}</p>${sections}<button class="secondary-btn" data-daily-listen="${key}">🔊 Read lesson aloud</button><div class="mini-check"><h4>Today's hands-on challenge</h4><p>${escapeHtml(l.challenge)}</p><p>Explain, draw or model your answer. This activity is not automatically scored.</p></div></div>${(pack.learningNotes||[]).map(n=>`<details class="lesson"><summary>${escapeHtml(n.title)}</summary><p>${escapeHtml(n.text)}</p><p>${escapeHtml(n.example)}</p></details>`).join("")}<button class="primary-btn lesson-practice" data-practice="${key}">Practice this day · ${pack.questions.length} mixed questions</button><details class="lesson"><summary>Browse the full year's topics</summary><ol>${topics.map((topic,i)=>`<li><button class="topic-link" data-topic-day="${i*28+1}">${escapeHtml(topic)} · Days ${i*28+1}–${i===12?365:(i+1)*28}</button></li>`).join("")}</ol></details><details class="lesson"><summary>Complete lesson library</summary>${library.map(item=>`<details class="lesson"><summary>${escapeHtml(item.title)}</summary><p>${escapeHtml(item.text)}</p><p>${escapeHtml(item.example)}</p><details><summary>${escapeHtml(item.try)}</summary><p>${escapeHtml(item.answer)}</p></details></details>`).join("")}</details></article>`;
 }).join("");
...
$("resetProgress").addEventListener("click",()=>{if(confirm("Clear scores from this device? Cloud scores will return the next time you synchronize.")){localStorage.removeItem("aqsaRecords");renderDashboard();renderHome()}});window.addEventListener("resize",()=>{if($("progressPage").classList.contains("active"))renderDashboard()});
