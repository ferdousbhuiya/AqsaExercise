-- Run this once in Supabase SQL Editor when profile sign-in works but score sync fails.
-- It preserves all existing profiles and scores.
...
begin
  ...
  select
    v_profile_id,left(x.id,100),x.date,x.timestamp,x.grade,x.subject,
  ...
  where x.id is not null and char_length(x.id)>=8
    and x.timestamp>0 and x.grade in (1,2)
    and x.subject in ('math','english','science')
    and x.score between 0 and 100
